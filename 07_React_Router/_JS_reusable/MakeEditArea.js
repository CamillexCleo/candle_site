...
function MakeEditArea({ inputSpecs, successCallBack, cancelCallBack, editObj = {}, title="Untitled" }) {
    ...
    editDiv.innerHTML = `
    <div class= "titleC"><h1>${title}</h1></div>
    <div class="rowsC"></div>
    <div class="radioHolderC"></div>
    <div class= "selectHolderC"></div>
    </div>
    ...
    var radioHolder = editDiv.getElementsByClassName("radioHolderC")[0];
    var selectHolder = editDiv.getElementsByClassName("selectHolderC")[0];
    ...
    */
    ...
    saveButton.onclick = function () {
        ...
        */
        var allErrors = "";
        for (var spec of inputSpecs) {
            console.log("to validate " + spec.inputTag.value +
                " as a " + spec.dataType);
            if (spec.dataType === "string") {
                spec.errorMsg.innerHTML = Validate.String(spec.inputTag.value,
                    spec.isRequired, spec.minLen, spec.maxLen)
                allErrors += spec.errorMsg.innerHTML;
            } else if (spec.dataType === "number") { // some other data type
                // call a different Validate method, 
                // passing in the values expected. 
                // set spec.errorMsg.innerHTML
                // allErrors += spec.errorMsg.innerHTML;
                spec.errorMsg.innerHTML = Validate.String(spec.inputTag.value,
                    spec.isRequired, spec.minLen, spec.maxLen)
                allErrors += spec.errorMsg.innerHTML;
            ...
            // For now, set "" (means no error). 
                // But eventually change this error message to something 
                // like 'Unknown data type'. 
